...
export function productInquiryText(product: Product): string {
  return `Hi Generation Bread! I'm interested in the ${product.name} (${
...
export function orderMessageText(order: Order): string {
  const lines: string[] = [
    `Hi Generation Bread! I'd like to confirm my order.`,
    ``,
    ...
    `Fulfillment: ${order.fulfillment === "pickup" ? "Store Pickup" : "Delivery"}`,
    `Fee: ${order.fulfillment === "pickup" ? "Free (pickup)" : "calculated upon confirmation"}`,

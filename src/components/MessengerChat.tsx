...
const STOP_WORDS = new Set([
  ...
  "please", "generation", "bread", "generationbread",
]);

const INTENTS: { test: RegExp; faq: RegExp }[] = [
  { test: /ship|deliver|freight|padala|pickup|pick-up/, faq: /deliver|pickup/ },
  { test: /return|exchange|refund|palit/, faq: /return|exchange/ },
  { test: /payment|gcash|maya|cod|bank|pay/, faq: /payment|method/ },
  { test: /wholesale|bulk|resell|custom|cake order/, faq: /bulk|wholesale|custom/ },
  { test: /where|location|located|address|based/, faq: /where|based/ },
  { test: /allergen|allergy|nut|gluten/, faq: /allergen/ },
  { test: /hour|open|close|schedule|bukas/, faq: /hour|operating/ },
  { test: /fresh|baked|daily/, faq: /fresh|baked/ },
...
export default function MessengerChat() {
  ...
  const messengerLink = messengerUrl(
    ...
    "Hi Generation Bread! I have a question from your website."
  );
  ...
  function productStock(p: ChatProduct) {
    ...
    botSay(`${p.name} is in stock and ready for pickup or delivery.`, {
      ctas: [{ label: "View product", href: `/products/${p.slug}` }],
      chips: FOLLOW_UPS,
    });
  }
  ...
  async function generalPrice(products: ChatProduct[]) {
    if (!products.length) {
      botSay("Check the shop for current prices — the menu rotates with fresh bakes.", {
    ...
    botSay(
      `Prices range from ${formatPeso(min)} to ${formatPeso(max)} across ${products.length} items — fresh bakes for every craving.`,
      {
        ctas: [{ label: "Browse products", href: "/products" }],
        chips: FOLLOW_UPS,
      }
  ...
  function categoryPrice(category: string, products: ChatProduct[]) {
    ...
    botSay(
      `${titleCaseSlug(category)} run ${range} across ${items.length} item${items.length !== 1 ? "s" : ""}.`,
      {
  ...
  function recommend(products: ChatProduct[]) {
    if (!products.length) {
      botSay("Browse the shop — fresh bakes land daily.", {
        ctas: [{ label: "Browse products", href: "/products" }],
        chips: FOLLOW_UPS,
      });
      return;
  ...
  async function handleUserInput(raw: string) {
    ...
    if (/stock|available|availability|sold\s*out|ready\s*to\s*ship/.test(lower)) {
      ...
      botSay(
        "Stock moves fast — most bakes are available daily. Ask about a specific item (e.g. “Ube Cheese Pandesal”) for its availability.",
        {
          ctas: [{ label: "Browse products", href: "/products" }],
          chips: FOLLOW_UPS,
        }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      botSay(
        `Hi! I'm the Generation Bread assistant. Ask me about the menu, prices, delivery, and more.`,
        {
